...
class ListenerThread(threading.Thread):
    def __init__(self, input_device, max_sound_samples, group=None, target=None, name=None, args=(), kwargs=None,
                 verbose=None):
    ...
    def run(self):
        ...
        for num_tries in range(num_retries):
            try:
                rospy.loginfo(rospy.get_caller_id() + ": starting audio listener for input device " + str(
                    self.input_device) + " try " + str(num_tries + 1))
        ...
        if self.stream is None:
            rospy.loginfo(rospy.get_caller_id() + ": failed to start audio listener for input device " + str(
                self.input_device) + " after " + str(num_retries) + " tries")
            exit(1)
    ...
    def record(self):
        ...
        while not has_min_voiced_frames and not self.is_shutdown:
            ...
            while not self.is_shutdown and len(r) < self.max_sound_samples:
                ...
                snd_data = array('h', self.stream.read(CHUNK_SIZE, exception_on_overflow=False))
...
class ListenNode(object):
    def __init__(self):
        ...
        self.model_name = self.get_param('~model', 'medium')

        rospy.loginfo('Loading whisper model')
        whisper_device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model = whisper.load_model(self.model_name).to(whisper_device)
    ...
    def publish_speech_segment(self, segment):
        ...
        result = self.model.transcribe(self.tmp_wav_file)
        text = result["text"]
    ...
    def write_wav_file(self, segment, path):
        ...
        if size > self.max_sound_samples:
            rospy.loginfo(rospy.get_caller_id() + ": truncating sound length " + str(size) + " to " + str(
                self.max_sound_samples) + " samples")
            # keep the most recent samples (at the end)
            segment.data = segment.data[-(self.max_sound_samples + 1):]
            size = len(segment.data)

        rospy.loginfo(rospy.get_caller_id() + ": writing sound length " + str(size) + " width " + str(
            segment.sample_width) + " to " + path)
